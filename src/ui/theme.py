...
def inject_theme() -> None:
    ...
    st.markdown(
        ...
        html, body, [class*="css"] {{ font-family:'Inter','Segoe UI',Arial,sans-serif; color:var(--atlas-text); }}
        .stApp {{ background:#F7F8FA; color:var(--atlas-text); }}
        .block-container {{ max-width:1500px; padding-top:1.1rem; padding-bottom:2.2rem; }}
        [data-testid="stSidebar"] {{ background:linear-gradient(180deg,#0E3038 0%,#0A252C 100%); border-right:1px solid rgba(255,255,255,.08); }}
        [data-testid="stSidebar"] > div:first-child {{ padding-top:1rem; }}
        [data-testid="stSidebar"] * {{ color:#EAF7F6; }}
        .atlas-brand {{ display:flex; align-items:center; gap:.75rem; padding:.35rem .25rem 1rem; }}
        .atlas-brand-mark {{ width:38px;height:38px;border-radius:11px;background:linear-gradient(135deg,var(--petronas-green),var(--petronas-lime));display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:1rem;box-shadow:0 8px 20px rgba(0,161,156,.22); }}
        ...
        .nav-section-label {{ margin:.9rem .25rem .4rem;font-size:.67rem;font-weight:700;letter-spacing:.11em;text-transform:uppercase;color:rgba(234,247,246,.45); }}
        ...
        .atlas-card,.atlas-card-white {{ background:#fff;border:1px solid var(--atlas-border);border-radius:16px;padding:1rem 1.05rem;box-shadow:0 5px 18px rgba(16,24,40,.035); }}
        .kpi-card {{ position:relative;background:#fff;border:1px solid var(--atlas-border);border-radius:16px;padding:1rem 1.05rem .95rem;min-height:112px;box-shadow:0 5px 18px rgba(16,24,40,.035);overflow:hidden; }}
        ...
        .micro-kpi {{ background:#fff;border:1px solid var(--atlas-border);border-radius:14px;padding:.75rem .85rem;min-height:82px;box-shadow:0 4px 12px rgba(16,24,40,.028); }}
        ...
        .micro-kpi.accent {{ border-top:3px solid var(--petronas-green); }} .micro-kpi.blue {{ border-top:3px solid var(--petronas-blue); }} .micro-kpi.lime {{ border-top:3px solid var(--petronas-lime); }} .micro-kpi.warning {{ border-top:3px solid var(--petronas-yellow); }}
        .status-strip {{ display:flex;justify-content:space-between;align-items:center;gap:.7rem;padding:.62rem .8rem;border-radius:12px;background:rgba(0,161,156,.055);border:1px solid rgba(0,161,156,.11);margin:.4rem 0; }}
        ...
        .status-pill.ready {{ background:rgba(52,211,153,.12);color:#A7F3D0; }} .status-pill.warn {{ background:rgba(251,191,36,.14);color:#FDE68A; }} .status-pill.error {{ background:rgba(248,113,113,.14);color:#FECACA; }}
        .decision-card {{ background:linear-gradient(135deg,#102F37 0%,#124C55 100%);border-radius:18px;padding:1.25rem;color:#fff;box-shadow:0 10px 28px rgba(15,57,65,.13); }}
        .decision-card .label {{ color:rgba(255,255,255,.7);font-size:.68rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase; }} .decision-card .value {{ font-size:1.7rem;font-weight:700;margin-top:.35rem; }} .decision-card .meta {{ color:rgba(255,255,255,.78);font-size:.77rem;margin-top:.35rem; }}
        .insight-card {{ border-radius:15px;padding:.9rem 1rem;border:1px solid var(--atlas-border);background:#fff;min-height:102px; }}
        .insight-tag {{ display:inline-block;font-size:.63rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--petronas-purple);margin-bottom:.25rem; }} .insight-title {{ color:#162F38;font-weight:700;font-size:.88rem; }} .insight-body {{ color:var(--atlas-muted);font-size:.76rem;margin-top:.2rem;line-height:1.45; }}
        ...
        [data-testid="stMetric"] [data-testid="stMetricDelta"] {{ color:#667085 !important; }}
