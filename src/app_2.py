...
class ExcelScreeningService:
    ...
    def screen(
        ...
        water_cut = _safe_float(v.get("water_cut_pct"), 0) or 0
        ...
        hc_source_ok = (
            self._in(v.get("hc_source_status"), ["Available", "Conditional"])
            or self._yes(v.get("hc_gas_availability"))
        )
        gas_source_ok = self._yes(v.get("gas_availability")) or hc_source_ok

        gor_category = str(v.get("produced_gor_category") or "")
        gor_weight = 1.0 if gor_category == "High" else 0.5 if gor_category == "Moderate" else 0.0
        source_conditional = (
        ...
        score = 100 * sum([
            ...
            self._yes(v.get("mmp_satisfied")), waterflood, mobility_ok,
        ]) / 7
        ...
        for bad, msg in [
            ...
            if bad:
                causes.append(msg)
        if gravity_high:
            conditional.append("Conditional: gravity override risk high")
        score = 100 * sum([
        ...
        results.append(self._result("CO2 WAG", bool(causes), causes, score, 18, v, conditional))
        ...
        if source_conditional:
            conditional.append("Conditional: produced GOR supports HC gas reinjection/recycling")
        score = 100 * sum([
            api >= 25, visc <= 10, depth >= 2500,
            self._yes(v.get("mmp_satisfied")), hc_source_ok,
            waterflood, mobility_ok, gor_weight,
        ]) / 8
        results.append(self._result("HC Gas Miscible Flood", bool(causes), causes, score, 14, v, conditional))
        ...
        if gravity_high:
            conditional.append("Conditional: gravity override risk high")
        score = 100 * sum([
            api >= 25, visc <= 10, self._yes(v.get("mmp_satisfied")),
            hc_source_ok, water_fac, gas_fac, waterflood, gas_inj,
            water_inj, gravity_ok, gor_weight,
        ]) / 11
        results.append(self._result("HC Gas WAG", bool(causes), causes, score, 17, v, conditional))
        ...
        for bad, msg in [
            ...
            if bad:
                causes.append(msg)
        if source_conditional:
            conditional.append("Conditional: produced GOR supports gas recycling")
        score = 100 * sum([
            api >= 15, visc <= 35, gas_source_ok, waterflood,
            mobility_ok, gor_weight,
        ]) / 6
        results.append(self._result("Immiscible Gas Flood", bool(causes), causes, score, 8, v, conditional))
        ...
        results.append(self._result("Immiscible Gas WAG", bool(causes), causes, score, 12, v, conditional))
        ...
        for bad, msg in [
            ...
            if bad:
                causes.append(msg)
        score = 100 * sum([
            api >= 22, visc <= 30, temp <= 90, tds <= 10000, hardness <= 50,
            self._eq(v.get("rock_type"), "Sandstone"),
        ...
        ranked = sorted(enumerate(results), key=lambda x: (-x[1]["Score (%)"], x[0]))
...
def render_excel_screening_result(result: Dict[str, Any]) -> None:
    results = result["results"]
    st.subheader("📋 EOR Screening Results")

    c1, c2, c3 = st.columns(3)
    c1.metric("🟢 PASS", sum(r["Status"] == "PASS" for r in results))
    c2.metric("🟡 CONDITIONAL", sum(r["Status"] == "CONDITIONAL" for r in results))
    c3.metric("🔴 FAIL (critical)", sum(r["Status"] == "FAIL (critical)" for r in results))

    table = pd.DataFrame(results).copy()
    table["Status"] = table["Status"].map(_screening_status_display)
    for col in ["Score (%)", "ΔRF_EOR (%)", "Final RF (%)", "EUR (MMstb)"]:
...
def render_eor_input_form(prefix: str) -> Tuple[Dict[str, Any], str]:
    ...
    with st.expander("📝 Reservoir Characteristics", expanded=True):
        ...
        with a:
            ...
            depth_ft = st.number_input("Depth (ft)", min_value=0.0, value=d["depth_ft"], step=50.0, key=f"{prefix}_depth")
            st.number_input("Reservoir Pressure (psia)", min_value=0.0, value=d["reservoir_pressure"], step=50.0, key=f"{prefix}_pressure")
            temperature_c = st.number_input("Temperature (°C)", min_value=0.0, value=d["temperature_c"], step=1.0, key=f"{prefix}_temperature")
            visc_cp = st.number_input("Oil Viscosity (cP)", min_value=0.0, value=d["visc_cp"], step=0.1, key=f"{prefix}_visc")
        with b:
            api = st.number_input("API Gravity (°API)", 0.0, 80.0, value=d["api"], step=0.5, key=f"{prefix}_api")
            perm_md = st.number_input("Permeability (mD)", min_value=0.0, value=d["perm_md"], step=10.0, key=f"{prefix}_perm")
            porosity_frac = st.number_input("Porosity (fraction)", 0.0, 1.0, value=d["porosity_frac"], step=0.01, key=f"{prefix}_por")
            salinity_ppm = st.number_input("Salinity / TDS (ppm)", min_value=0.0, value=d["salinity_ppm"], step=1000.0, key=f"{prefix}_tds")
            hardness_ppm = st.number_input("Hardness Ca²⁺+Mg²⁺ (ppm)", min_value=0.0, value=d["hardness_ppm"], step=50.0, key=f"{prefix}_hardness")
        with c:
            rock_type = st.selectbox("Rock Type", ["Sandstone", "Carbonate", "Unconsolidated sands"], key=f"{prefix}_rock")
            net_pay_m = st.number_input("Net Pay (m)", min_value=0.0, value=d["net_pay_m"], step=1.0, key=f"{prefix}_netpay")
            ...
            so_pct = st.number_input("Oil Saturation (%) [CatBoost]", 0.0, 100.0, value=55.0, step=1.0, key=f"{prefix}_so")

    with st.expander("🛢️ Recovery / EOR Context", expanded=True):
        a, b, c = st.columns(3)
        with a:
            ooip_mmstb = st.number_input("OOIP (MMstb)", min_value=0.0, value=d["ooip_mmstb"], step=10.0, key=f"{prefix}_ooip")
            base_rf_pct = st.number_input("Base RF without EOR (%)", 0.0, 100.0, value=d["base_rf_pct"], step=1.0, key=f"{prefix}_base_rf")
            drive_mechanism = st.selectbox("Drive Mechanism", ["Waterflood", "Gas Drive", "Solution Gas Drive", "Natural Depletion", "Other"], key=f"{prefix}_drive")
    ...
    with st.expander("🔁 Injection / WAG / Gas Context", expanded=False):
        ...
        with c:
            ...
            produced_gor_scf_stb = st.number_input("Produced GOR (scf/STB)", min_value=0.0, value=d["produced_gor_scf_stb"], step=50.0, key=f"{prefix}_gor_numeric")

    values = {
        "depth_ft": depth_ft, "temperature_c": temperature_c, "visc_cp": visc_cp,
        "api": api, "perm_md": perm_md, "porosity_frac": porosity_frac,
        "porosity_pct": porosity_frac * 100.0, "so_pct": so_pct,
        "salinity_ppm": salinity_ppm, "hardness_ppm": hardness_ppm,
        "rock_type": rock_type, "net_pay_m": net_pay_m,
        "heterogeneity": heterogeneity, "gas_availability": gas_availability,
        "oxygen_present": oxygen_present, "ooip_mmstb": ooip_mmstb,
        "base_rf_pct": base_rf_pct, "drive_mechanism": drive_mechanism,
        "drive_multiplier": drive_multiplier, "co2_availability": co2_availability,
        "mmp_satisfied": mmp_satisfied, "waterflood_history": waterflood_history,
        "water_cut_pct": water_cut_pct, "field_maturity": field_maturity,
        "maturity_factor": maturity_factor, "mobility_ratio": mobility_ratio,
        "adsorption_risk": adsorption_risk, "water_handling": water_handling,
        "offshore": offshore, "water_injection_facilities": water_injection_facilities,
        "gas_injection_facilities": gas_injection_facilities, "wag_ratio": wag_ratio,
        "gas_injectivity": gas_injectivity, "water_injectivity": water_injectivity,
        "gravity_override_risk": gravity_override_risk, "hc_gas_availability": hc_gas_availability,
        "produced_gor_category": produced_gor_category, "gas_reinjectable": gas_reinjectable,
...
def build_eor_ml_input(values: Dict[str, Any], formation: str) -> Dict[str, Any]:
    """Convert EOR Atlas UI values into the exact range-based input schema expected by CatBoost."""
    formation_map = {"Sandstone": "Sandstone", "Carbonate": "Carbonates", "Carbonates": "Carbonates", "Unconsolidated sands": "Unconsolidated sands"}
    if formation not in formation_map:
        raise ValueError(f"Formation '{formation}' is not supported by the active CatBoost model. Supported formations: {list(formation_map.keys())}")

    required_keys = ["depth_ft", "porosity_pct", "perm_md", "api", "visc_cp", "so_pct"]
    missing = [key for key in required_keys if key not in values]
    if missing:
        raise KeyError(f"Missing EOR Intelligence inputs: {missing}. Available keys: {list(values.keys())}")
    ...
    if depth <= 0: raise ValueError("Depth must be greater than zero.")
    if not 0 <= porosity <= 100: raise ValueError("Porosity must be between 0 and 100%.")
    if permeability <= 0: raise ValueError("Permeability must be greater than zero.")
    if not 0 <= api <= 100: raise ValueError("API gravity must be between 0 and 100°.")
    if viscosity <= 0: raise ValueError("Viscosity must be greater than zero.")
    if not 0 <= oil_saturation <= 100: raise ValueError("Oil saturation must be between 0 and 100%.")

    return {
        "depth_min_ft": depth, "depth_max_ft": depth,
        "porosity_min_pct": porosity, "porosity_max_pct": porosity,
        "perm_min_md": permeability, "perm_max_md": permeability,
        "api_min": api, "api_max": api,
        "visc_min_cp": viscosity, "visc_max_cp": viscosity,
        "so_min_pct": oil_saturation, "so_max_pct": oil_saturation,
...
# EOR INTELLIGENCE
# =============================================================================
def run_eor_intelligence(services: Dict[str, Any], values: Dict[str, float], formation: str) -> Dict[str, Any]:
    ...
    fuzzy_scores = fuzzy_engine.evaluate_all(techs_all, formation, values)
    fuzzy_top5 = sorted(fuzzy_scores.items(), key=lambda item: item[1], reverse=True)[:5]
    ...
    if model_service.is_loaded():
        ...
        if not ml_result.success:
            raise RuntimeError(ml_result.warning or "CatBoost prediction failed.")
        ml_top3 = ml_result.top_n(3)
        ml_probabilities = {candidate.technique: candidate.probability for candidate in ml_result.candidates}

    return {
        "formation": formation, "fuzzy_scores": fuzzy_scores, "fuzzy_top5": fuzzy_top5,
        "ml_available": model_service.is_loaded(), "ml_top3": ml_top3,
...
def render_eor_intelligence_result(result: Dict[str, Any]) -> None:
    ...
    if result["ml_available"]:
        ...
        for col, candidate in zip(cols, result["ml_top3"]):
            with col:
                st.markdown(
                    f'<div class="top3-card"><div style="font-size:0.72rem;color:#64748b;">RANK {candidate.rank}</div><div style="font-size:1.2rem;font-weight:700;">{candidate.technique}</div><div>CatBoost probability: <strong>{_format_probability(candidate.probability)}</strong></div></div>',
                    unsafe_allow_html=True,
                )
        ml_df = pd.DataFrame([
            {"Rank": c.rank, "EOR Technique": c.technique, "CatBoost Probability": c.probability}
            for c in result["ml_top3"]
        ])
        ml_df["CatBoost Probability"] = ml_df["CatBoost Probability"].map(_format_probability)
        st.dataframe(ml_df, use_container_width=True, hide_index=True)
    else:
        st.warning("CatBoost model is unavailable.")
        st.markdown("### 🌐 Fuzzy Suitability Top 5")
        fuzzy_df = pd.DataFrame([
            {"Rank": i, "EOR Technique": technique, "Fuzzy Suitability": round(float(score), 3)}
            for i, (technique, score) in enumerate(result["fuzzy_top5"], start=1)
        ])
    ...
    if result["ml_probabilities"]:
        ...
        st.bar_chart(pd.Series(result["ml_probabilities"], dtype=float).sort_values(ascending=False))

    st.markdown("### Fuzzy Suitability Distribution")
    if result["fuzzy_top5"]:
        st.bar_chart(pd.Series(dict(result["fuzzy_top5"]), dtype=float))

    st.info("No weighted ML/fuzzy ensemble is applied. Use the deterministic screening result for engineering feasibility.")
...
def render_eor_screening_tab() -> None:
    ...
    st.write("Deterministic reservoir screening only. This layer does not call CatBoost or fuzzy scoring.")
...
def render_executive_overview_section():
    ...
    metrics = [
        ...
        ("Past EOR Studies", 128), ("RF Gap (MMstb)", 1.31), ("Potential EOR Candidates", 18),
    ]
    cols = st.columns(6)
    for col, (label, value) in zip(cols, metrics):
        with col:
            st.markdown(
                f"""<div class="exec-kpi-card"><div class="exec-kpi-label">{label}</div><div class="exec-kpi-value">{value}</div></div>""",
                unsafe_allow_html=True,
            )
...
def render_field_candidates_section():
    """Render Candidate Screening as a peer tab inside the same dashboard."""
    render_field_reservoir_parameters_tab(workbook_path=str(settings.workbook_path))


def render_fluid_fluid_section():
    st.header("🧪 CEOR — Fluid / Fluid")
    rheology = pd.DataFrame({"Shear_Rate": [10, 30, 50, 100, 200], "Polymer_A": [120, 90, 75, 58, 44], "Polymer_B": [132, 104, 82, 64, 49], "Polymer_C": [110, 88, 70, 54, 41]}).set_index("Shear_Rate")
    st.subheader("Rheology")
    st.line_chart(rheology)
    thermal = pd.DataFrame({"Time_Days": [0, 7, 14, 30, 60], "Retention_A": [100, 96, 90, 84, 79], "Retention_B": [100, 92, 85, 75, 68]}).set_index("Time_Days")
    st.subheader("Thermal Stability")
    st.line_chart(thermal)
    phase = pd.DataFrame({"Formulation": ["A-1", "A-2", "B-1", "B-2", "C-1"], "No_Precipitation": [82, 70, 65, 91, 76], "Precipitation": [18, 30, 35, 9, 24]})
    st.subheader("Phase Behaviour")
    st.bar_chart(phase.set_index("Formulation"))
    ift = pd.DataFrame({"Formulation": ["F1", "F2", "F3", "F4"], "IFT": [0.022, 0.010, 0.040, 0.013]})
...
def render_fluid_rock_section():
    ...
    adsorption = pd.DataFrame({"Days": [0, 7, 14, 30, 60], "Adsorption_Surf_A": [0.0, 0.18, 0.27, 0.35, 0.42], "Adsorption_Surf_B": [0.0, 0.12, 0.20, 0.28, 0.31]}).set_index("Days")
    st.subheader("Adsorption vs Time")
    st.line_chart(adsorption)
    coreflood = pd.DataFrame({"Core": ["Core 1", "Core 2", "Core 3", "Core 4"], "Waterflood": [42, 38, 45, 41], "EOR_Increment": [15, 18, 11, 14]})
    st.subheader("Core Flood Incremental Recovery")
    st.bar_chart(coreflood.set_index("Core"))
    sor = pd.DataFrame({"Core": ["Core 1", "Core 2", "Core 3", "Core 4"], "Sor_Reduction": [36, 28, 41, 31]})
...
def render_challenges_section():
    ...
    df = pd.DataFrame({"Field": ["Angsi", "Dulang", "Barton", "Baram"], "Chemical_Compatibility": ["High", "Medium", "Low", "High"], "Facilities": ["Medium", "Low", "High", "Medium"], "Fluid_Handling": ["High", "Medium", "Medium", "Low"], "Cost_Logistics": ["High", "Medium", "Low", "High"]})
    st.dataframe(df, use_container_width=True, hide_index=True)
    dist = pd.DataFrame({"Challenge": ["Chemical compatibility", "Facilities", "Fluid handling", "Cost / logistics", "Risk / uncertainty"], "Count": [4, 3, 3, 2, 2]})
...
def render_database_summary_section():
    ...
    cols[1].metric("Model Status", "Ready" if settings.validate_paths().get("model") else "Missing")
    cols[2].metric("Workbook", "Loaded" if settings.validate_paths().get("workbook") else "Missing")
    cols[3].metric("Environment", settings.environment.upper())
    if recent:
        rows = [{"Time": run.timestamp.strftime("%Y-%m-%d %H:%M") if run.timestamp else "N/A", "Formation": run.formation, "Recommendation": run.recommended_technique or "N/A", "Score": run.recommendation_score} for run in recent[:10]]
...
def render_saved_run_detail_section():
    ...
    selected_id = st.selectbox("Select a saved run", run_ids, key="saved_run_detail")
    ...
    with left:
        ...
        st.write(f"**Recommendation:** {selected.recommended_technique or 'N/A'}")
    with right:
        st.write(f"**Status:** {selected.recommendation_status or 'N/A'}")
        st.write(f"**Score:** {selected.recommendation_score}")
        st.write(f"**Model Version:** {selected.model_version or 'N/A'}")
        st.write(f"**Data Readiness:** {selected.data_readiness_pct or 'N/A'}%")
    if selected.input_payload:
        st.write("**Exact input values:**")
        st.dataframe(pd.DataFrame([selected.input_payload]), use_container_width=True, hide_index=True)
...
def main():
    ...
    with tabs[7]:
        st.header("🤖 EOR Intelligence with CatBoost Machine Learning Classification Method")
        st.write("Uses the same reservoir inputs as EOR Screening. CatBoost and fuzzy suitability are independent signals.")
        ...
        if st.button("🧠 Run EOR Intelligence", type="primary", use_container_width=True, key="run_eor_intelligence"):
            ml_values = {
                "depth_ft": inputs["depth_ft"], "porosity_pct": inputs["porosity_pct"],
                "perm_md": inputs["perm_md"], "api": inputs["api"],
                "visc_cp": inputs["visc_cp"], "so_pct": inputs["so_pct"],
            }
            with st.spinner("Running fuzzy and CatBoost intelligence..."):
                try:
                    result = run_eor_intelligence(services, ml_values, formation)
