...
def render(services: dict) -> None:
    ...
    inputs_state = st.session_state.get("intel_last_inputs") or {}
    kpi_cards([
        ...
        ("Fusion", "65 / 35", "ML / engineering weighting"),
    ])
    ...
    with input_tab:
        ...
        with action_col:
            reset_button("↺ Reset inputs", prefixes="intel_", result_keys=("eor_intelligence_result", "intel_last_inputs"), key="intelligence_reset_inputs")
        section_title("Reservoir Context", "Complete the same v3 schema used by Engineering Screening. No Excel Gate, CatBoost, or Decision Fusion calculation runs while fields are edited.")
        ...
        if submitted:
            try:
                with st.status("Running Hybrid Intelligence…", expanded=True) as run_status:
                    st.write("Engineering Gate → CatBoost → Decision Fusion")
                    _hybrid._run_intelligence_from_current_inputs(services, inputs, formation)
                    st.session_state["intel_last_inputs"] = inputs.copy()
                    st.session_state["intel_last_formation"] = formation
    ...
    with decision_tab:
        if not result:
            ...
            return
        mode = view_mode("intelligence_view_mode")
        hybrid = result.get("hybrid", {}) or {}
        recommendation = hybrid.get("recommendation")
        ranking = hybrid.get("ranking", []) or []
        quality = "HIGH" if result.get("ml_available") and inputs_state else ("MEDIUM" if inputs_state else "LIMITED")
        decision_hero(recommendation, quality)

        if mode == "Leadership":
            kpi_cards([
                ("Recommendation", recommendation.get("EOR Technique", "—") if recommendation else "—", "Top hybrid outcome"),
                ("Hybrid score", f"{float(recommendation.get('Hybrid Score', 0)):.1%}" if recommendation else "—", "Combined decision signal"),
                ("Gate", recommendation.get("Engineering Status", "—") if recommendation else "—", "Engineering anchor"),
                ("ML", "Available" if result.get("ml_available") else "Unavailable", "CatBoost runtime state"),
            ])
            with st.expander("Why / Why Not", expanded=True):
                why_why_not(ranking)
            with st.expander("Decision Trace", expanded=False):
                decision_trace(result.get("screening"), result.get("ml_top3"), ranking)
            ...
            with st.expander("ScreenTool v3 Context", expanded=False):
            ...
            with st.expander("Decision Fusion Configuration", expanded=False):
