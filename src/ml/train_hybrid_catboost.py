...
IMPORTANT:
    This pipeline requires a labeled historical dataset with an actual EOR
...
this richer labeled dataset is assembled and this trainer produces validated
v2 artifacts.
"""
...
def cross_validate(X: pd.DataFrame, y: np.ndarray, params: dict[str, Any], folds: int = 5) -> dict[str, float]:
    ...
    splitter = StratifiedKFold(n_splits=actual_folds, shuffle=True, random_state=56)
    accuracies = []
    macro_f1s = []
...
def train_and_save(dataset_path: Path, output_dir: Path, version: str = "2.0.0") -> dict[str, Any]:
    ...
    validation = cross_validate(X, y, params)
    ...
    config = {
        ...
        "warning": (
            "Validation statistics are only meaningful when the dataset is large enough "
            "and split methodology reflects field-level generalization."
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument("dataset", type=Path, help="Labeled historical EOR dataset (.xlsx/.csv).")
    parser.add_argument(
        "--output-dir",
        type=Path,
